...
test("leaderboard names are cleaned, filtered, and bounded", () => {
...
test("profile updates rename historical leaderboard rows", async (t) => {
  ...
  const response = await handleLeaderboardRequest(
    new Request("https://worker.example/leaderboard-profile", {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: "https://moviemaster.vip" },
      body: JSON.stringify({ visitorId: VISITORS[0], name: "MOVIE APPRENTICE" }),
    }),
    { DB: db },
  );
